...
export function EventCard({ event }: { event: EventDto }) {
    ...
    // Проверяем, наступила ли уже дата мероприятия
    const isPast = new Date(event.date) < new Date();

    return (
        <Link
            to={`/events/${event.id}`}
            // Добавляем классы grayscale и opacity-60, если событие прошло
            className={`group relative flex flex-col overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm transition-all duration-200 ${
                isPast
                    ? 'opacity-60 grayscale hover:opacity-80 hover:shadow-md'
                    : 'hover:-translate-y-1 hover:border-indigo-200 hover:shadow-xl hover:shadow-indigo-900/[0.06]'
            }`}
        >
            {/* Заменяем яркий градиент на серый для прошедших событий */}
            <div className={`h-1.5 opacity-90 ${isPast ? 'bg-gray-300' : 'bg-gradient-to-r from-indigo-600 via-violet-500 to-fuchsia-500'}`} />

            <div className="flex flex-1 gap-4 p-5">
                <div className={`flex h-fit w-14 shrink-0 flex-col items-center rounded-xl py-2 ${isPast ? 'bg-gray-100 text-gray-500' : 'bg-indigo-50 text-indigo-700'}`}>
                    ...
                    <span className="mt-0.5 text-[10px] opacity-80">{date.weekday.slice(0, 3)}</span>
                </div>
                <div className="flex min-w-0 flex-1 flex-col">
                    <div className="mb-1 flex items-start gap-2">
                        <h3 className={`line-clamp-2 flex-1 font-semibold transition-colors ${isPast ? 'text-gray-600' : 'text-gray-900 group-hover:text-indigo-700'}`}>
                            {event.title}
                        </h3>
                        {statusTone && !isPast && (
                        ...
                        {/* Дополнительный бейдж для прошедших событий */}
                        {isPast && (
                            <Badge tone="gray">Прошло</Badge>
                        )}
                    </div>
            ...
            <div className="relative border-t border-dashed border-gray-200 px-5 py-3">
              ...
              {event.venueName} • {event.venueCity}
                    ...
                    <div className={`shrink-0 text-sm font-bold ${isPast ? 'text-gray-400' : 'text-gray-900'}`}>
                        {event.priceMin === 0 && event.priceMax === 0 ? (
                            <span className="font-semibold text-gray-400">Бесплатно</span>
                        ) : (
                            <>
