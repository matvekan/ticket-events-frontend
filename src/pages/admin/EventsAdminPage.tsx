...
import { EVENT_STATUS_LABELS, EVENT_STATUS_TONES, formatDateTime, formatPrice, SEAT_TYPE_LABELS } from '../../lib/format';

const eventSchema = z.object({
    title: z.string().min(3, 'Слишком короткое название').max(100),
...
export function EventsAdminPage() {
    ...
    const { data, isLoading: loadingEvents, hasNextPage, fetchNextPage, isFetchingNextPage } = useAdminEvents();
    ...
    const { register, handleSubmit, reset, watch, formState: { errors, isSubmitting } } = useForm<EventForm>({ resolver: zodResolver(eventSchema) });
    ...
    const availableTypes = useMemo(() => {
        if (!venueSeats) return [];
        return Array.from(new Set(venueSeats.map(seat => seat.type)));
    }, [venueSeats]);

    const onSubmit = async (values: EventForm) => {
        if (!venueSeats || venueSeats.length === 0) {
            setError('В выбранной площадке нет мест.');
            return;
        }

        const missingTypes = availableTypes.filter(type => !pricesByType[type] || pricesByType[type].trim() === '');
        if (missingTypes.length > 0) {
            setError('Пожалуйста, укажите цену для всех типов мест на площадке.');
            return;
        }

        const seats = venueSeats.map(seat => ({
            seatId: seat.id,
            priceAmount: Math.round(Number(pricesByType[seat.type]) * 100)
        }));
    ...
    return (
        <div className="grid gap-8 lg:grid-cols-2">
            <div className="h-fit rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
                <h2 className="mb-4 text-lg font-semibold text-gray-900">Новое мероприятие</h2>
                {error && (
                    <div className="mb-4">
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                    ...
                    {selectedVenue && (
                        <div className="rounded-lg border border-gray-200 bg-gray-50 p-4">
                            <div className="mb-2 text-sm font-medium text-gray-700">Цены на билеты</div>
                            {loadingSeats && <Spinner label="Загрузка схемы..." />}

                            {!loadingSeats && availableTypes.length > 0 && (
                                <div className="space-y-2">
                                    {availableTypes.map((type) => (
                                        <label key={type} className="flex items-center justify-between gap-3 rounded-md bg-white px-3 py-2 text-sm border border-gray-100 shadow-sm">
                                            ...
                                            <input
                                                ...
                                                onChange={(e) => setPricesByType((prev) => ({ ...prev, [type]: e.target.value }))}
                                                className="w-32 rounded-md border border-gray-300 px-3 py-1.5 text-right text-sm outline-none focus:border-indigo-500"
                                            />
                                        </label>
                                    ))}
                                </div>
                            ...
                            {!loadingSeats && availableTypes.length === 0 && (
                                <p className="text-sm text-gray-500">На этой площадке нет мест.</p>
                            )}
                        </div>
                    )}
                    <Button type="submit" className="w-full" loading={isSubmitting}>Создать мероприятие</Button>
                </form>
            </div>

            <div>
                <h2 className="mb-4 text-lg font-semibold text-gray-900">Мероприятия</h2>
                {loadingEvents && <Spinner />}
                {!loadingEvents && allEvents.length === 0 && (
                    <div className="rounded-2xl border border-dashed border-gray-300 bg-white p-10 text-center">
                        <p className="text-sm text-gray-500">Нет созданных мероприятий.</p>
                    </div>
                )}
