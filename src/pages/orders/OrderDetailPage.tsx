...
export function OrderDetailPage() {
  ...
  if (isError || !order) return <Alert variant="error">Заказ не найден.</Alert>;
  ...
  return (
    <div className="max-w-3xl">
      ...
      {paymentResult === 'success' && (
        <div className="mb-4">
          <Alert variant="success">Оплата прошла успешно! Билеты доступны в разделе «Мои заказы».</Alert>
        </div>
      )}

      {paymentResult === 'failed' && (
        <div className="mb-4">
          <Alert variant="error">Платёж отклонён банком. Вы можете попробовать оплатить заказ ещё раз.</Alert>
        </div>
      )}
        ...
        {order.tickets.map((ticket) => (
          <div key={ticket.id} className="flex items-center justify-between gap-4 border-b border-gray-100 p-4 last:border-b-0">
            <div className="min-w-0">
              ...
              <div className="mt-1 font-mono text-xs text-gray-400">Билет: {ticket.code}</div>
